...
import React, { useReducer, useEffect, useCallback, useMemo, useSyncExternalStore } from "react";
...
import { Loader2, Clock, User, ChevronLeft, Check, AlertCircle, Info, Tag, Scissors, ArrowRight } from 'lucide-react';
...
function bookingWizardReducer(state: BookingWizardState, action: BookingWizardAction): BookingWizardState {
switch (action.type) {
case "SET_STEP":
...
function ServiceStep({ services, selectedService, dispatch }: ServiceStepProps) {
return (
...
onClick={() => {
dispatch({ type: "SET_SELECTED_SERVICE", payload: s });
...
function StaffStep({ staff, selectedService, selectedStaff, dispatch }: StaffStepProps) {
return (
<div className="space-y-8">
<div className="space-y-2">
<h2 className="text-3xl font-bold tracking-tight text-foreground">
...
{staff.reduce<React.ReactNode[]>((acc, member) => {
                if (member.serviceIds && selectedService && !member.serviceIds.includes(selectedService.id)) return acc;
                acc.push(
                  <Card
                    key={member.id}
                    className={cn(
                    ...
                    onClick={() => {
                      dispatch({ type: "SET_SELECTED_STAFF", payload: member });
...
>
<ChevronLeft className="size-4 mr-2" /> Change
Service
</Button>
</div>
</div>
...
function ScheduleStep({ today, selectedDate, selectedTime, availableSlots, loadingAvailability, dispatch }: ScheduleStepProps) {
return (
<div className="space-y-8">
<div className="space-y-2">
<h2 className="text-3xl font-bold tracking-tight text-foreground">
...
>
<ChevronLeft className="size-4 mr-2" /> Back to
Expert
</Button>
...
step: preSelectedServiceId && initialServices.find((s) => s.id === preSelectedServiceId) ? 2 : 1,
selectedService: initialServices.find((s) => s.id === preSelectedServiceId) || null,
selectedStaff: null,
selectedDate: "",
selectedTime: "",
notes: "",
...
const { step, selectedService, selectedStaff, selectedDate, selectedTime, notes, submitting, error, availableSlots, loadingAvailability, couponCode, couponResult, validatingCoupon, couponError } = state;

const checkAvailability = useCallback(async () => {
if (!state.selectedStaff || !state.selectedDate || !state.selectedService) return;
try {
...
useEffect(() => {
if (state.selectedDate && state.selectedStaff && state.selectedService && state.step === 3) {
checkAvailability();
} else {
dispatch({ type: "SET_AVAILABLE_SLOTS", payload: [] });
}
}, [state.selectedDate, state.selectedStaff, state.selectedService, state.step, checkAvailability]);
...
dispatch({ type: "SET_COUPON_ERROR", payload: result.message || "Invalid coupon code" });
dispatch({ type: "SET_COUPON_RESULT", payload: null });
}
} catch (err: any) {
dispatch({ type: "SET_COUPON_ERROR", payload: err.message || "Failed to validate coupon" });
} finally {
...
const startTime = new Date(`${state.selectedDate}T${state.selectedTime}`);
...
dispatch({ type: "SET_ERROR", payload: err.message || "Failed to create appointment" });
} finally {
