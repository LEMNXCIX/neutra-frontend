...
import React, { useReducer, useEffect, useCallback, useSyncExternalStore } from "react";
    ...
    <TableRow>
      ...
      <TableCell className="text-right">
        <div className="flex justify-end gap-2">
          <Button variant="ghost" size="icon" onClick={() => onFeatures(tenant)} title="Manage Features">
            <Settings className="size-4" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => onEdit(tenant)}>
            <Edit className="size-4" />
          </Button>
          <Button variant="ghost" size="icon" onClick={() => onDelete(tenant)} className="text-destructive hover:text-destructive">
...
export function TenantsTable({ initialTenants = EMPTY_TENANTS }: TenantsTableProps) {
  ...
  {state.loading && state.tenants.length === 0 ? (
    <TableRow>
      <TableCell colSpan={5} className="text-center py-8">Loading…</TableCell>
              ...
              filteredTenants.map((tenant) => (
                <TenantRow key={tenant.id} tenant={tenant} onEdit={openEdit} onDelete={handleDelete} onFeatures={openFeatures} />
              ))
            )}
          </TableBody>
                        </Table>
    ...
    {state.loading && state.tenants.length === 0 ? (
      <div className="text-center py-8">Loading…</div>
            ...
            filteredTenants.map((tenant) => (
              <TenantCard key={tenant.id} tenant={tenant} onEdit={openEdit} onDelete={handleDelete} onFeatures={openFeatures} />
            ))
          )}
        </div>
                </CardContent>
  ...
  {state.managingFeaturesTenant && (
    <TenantFeaturesDialog
      ...
      onOpenChange={(open) => dispatch({ type: "SET_FEATURES_DIALOG_OPEN", payload: open })}
