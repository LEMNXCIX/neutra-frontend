...
interface TabContentProps {
  ...
  setFormData: React.Dispatch<React.SetStateAction<CreateTenantData>>;
  updateConfig: (section: "branding" | "settings" | "features", key: string, value: any) => void;
...
function GeneralTabContent({ formData, setFormData, isWizard, setActiveTab, tenant }: TabContentProps) {
  return (
    <TabsContent value="general" className="py-4 space-y-4">
      ...
      <div className="space-y-2">
        ...
        <p className="text-xs text-muted-foreground">
          Will be accessed at {formData.slug || "your-slug"}
          .xcix.ec
        </p>
      </div>
      <div className="space-y-2">
        ...
        >
          ...
          <SelectContent>
            ...
            <SelectItem value={TenantType.HYBRID}>
              Hybrid
            </SelectItem>
          </SelectContent>
        </Select>
...
function BrandingTabContent({ formData, updateConfig, isWizard, setActiveTab }: TabContentProps) {
  return (
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <Input
            ...
            value={
              formData.config?.branding?.primaryColor ||
              "#000000"
            }
            onChange={(e) =>
              updateConfig(
...
function SettingsTabContent({ formData, updateConfig, isWizard, setActiveTab }: TabContentProps) {
  return (
    <TabsContent value="settings" className="py-4 space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          <Input
            value={
              formData.config?.settings?.supportEmail ||
              ""
            }
            onChange={(e) =>
              updateConfig(
        ...
        <div className="space-y-2">
          ...
          <Input
            value={
              formData.config?.settings?.websiteUrl || ""
            }
            onChange={(e) =>
              updateConfig(
                "settings",
        ...
        <div className="space-y-2">
          ...
          <Select
            value={
              formData.config?.settings?.currency || "USD"
            }
            onValueChange={(val) =>
              updateConfig("settings", "currency", val)
            }
          >
            <SelectTrigger className="h-12">
        ...
        <div className="space-y-2">
          ...
          <Select
            value={
              formData.config?.settings?.language || "es"
            }
            onValueChange={(val) =>
              updateConfig("settings", "language", val)
            }
          >
            <SelectTrigger className="h-12">
...
function FeaturesTabContent({ formData, toggleFeature, isWizard, setActiveTab, platformFeatures }: TabContentProps) {
...
const [formData, setFormData] = useState<CreateTenantData>(() => ({
    name: tenant?.name || "",
    slug: tenant?.slug || "",
    type: tenant?.type || TenantType.STORE,
    ...
    const updateConfig = (
        section: "branding" | "settings" | "features",
        key: string,
        value: any,
    ) => {
        setFormData((prev) => ({
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
                <TabsList
                    className={`grid w-full ${isWizard ? "grid-cols-4" : "grid-cols-3"}`}
                >
        ...
        />
            ...
            <div className="flex justify-end gap-3 pt-6 border-t">
                ...
                <Button
                    type="submit"
                    disabled={isSaving}
                    className="px-8 font-bold shadow-lg shadow-primary/20 rounded-xl"
                >
                    {isSaving ? (
                        <>
