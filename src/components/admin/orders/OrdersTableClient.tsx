        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4 p-4 bg-muted/50 rounded-lg">
            <div>
              ...
              <p className="font-medium">{order.user?.name || order.userId}</p>
              {order.user?.email && <p className="text-xs text-muted-foreground">{order.user.email}</p>}
            </div>
            <div>
            ...
            <div>
              ...
              <p className="text-lg font-bold">${calculateTotal(order).toFixed(2)}</p>
            </div>
          </div>
          ...
          {order.address && (
            <div>
              ...
              <p className="text-sm p-3 bg-muted/50 rounded">{order.address}</p>
            </div>
          )}

          <div>
            <h3 className="font-semibold mb-2">Update Order Status</h3>
            <Select value={order.status} onValueChange={(newStatus) => onUpdateStatus(order.id, newStatus)} disabled={isUpdatingStatus === order.id}>
          ...
          <div>
            <h3 className="font-semibold mb-2 flex items-center gap-2"><Truck className="size-4" /> Tracking Number</h3>
            <div className="flex gap-2">
              <Input value={editingTracking} onChange={(e) => setEditingTracking(e.target.value)} placeholder="Enter tracking number..." />
              <Button onClick={() => onUpdateTracking(order.id, editingTracking)} disabled={isUpdatingTracking}>
                ...
                <TableBody>
                  {order.items?.map((item) => (
                    <TableRow key={item.productId}>
                      ...
                      <TableCell className="text-right font-medium">${(item.price * item.amount).toFixed(2)}</TableCell>
                    </TableRow>
                  ))}
                </TableBody>
              </Table>
          ...
          {order.coupon && (
            <div className="p-4 bg-green-50 dark:bg-green-900/20 rounded-lg border border-green-200 dark:border-green-800">
              <h3 className="font-semibold mb-1 text-green-900 dark:text-green-100">Coupon Applied</h3>
              <p className="text-sm text-green-700 dark:text-green-300">
                Code: <span className="font-mono font-bold">{order.coupon.code}</span> ({order.coupon.type === "percent" ? `${order.coupon.value}%` : `$${order.coupon.value}`} off)
              </p>
              <p className="text-sm text-green-700 dark:text-green-300">
                Discount: <span className="font-bold">-${order.coupon.discount.toFixed(2)}</span>
              </p>
            </div>
          )}
        </div>
...
function ordersDialogReducer(state: OrdersDialogState, action: OrdersDialogAction): OrdersDialogState {
  switch (action.type) {
...
function OrdersTableClientInner({
  ...
  React.useEffect(() => {
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (data.success && Array.isArray(data.data)) {
            dispatch({ type: "SET_STATUSES", payload: data.data });
          }
        }
      } catch (error) {
    ...
    const updateOrderStatus = async (orderId: string, newStatus: string) => {
        ...
        if (dialogState.selectedOrder && dialogState.selectedOrder.id === orderId) {
        dispatch({ type: "SET_SELECTED_ORDER", payload: {
    ...
    dispatch({ type: "SET_EDITING_TRACKING", payload: order.trackingNumber || "" });
  ...
  return (
    <div className="w-full space-y-6" suppressHydrationWarning>
      ...
      setEditingTracking={(v) => dispatch({ type: "SET_EDITING_TRACKING", payload: v })}
