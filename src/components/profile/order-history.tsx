                        ...
                        <CardHeader className="bg-muted/30 p-8 border-b border-border/50">
                            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                                <div className="flex flex-wrap items-center gap-3">
                                    ...
                                    <Badge
                                        className={`${getStatusColor(o.status)} rounded-full px-4 py-1 text-[10px] font-bold uppercase tracking-widest border-none shadow-sm`}
                                    >
                                        {o.status}
                                    </Badge>
                                </div>
                        ...
                        <CardContent className="p-8 space-y-6">
                            ...
                            <div className="flex gap-3">
                                <Button
                                    variant="outline"
                                    size="lg"
                                    className="rounded-xl font-bold border-2 px-6 h-12 hover:bg-muted"
                                    onClick={() =>
                                        setExpanded(
                                            expanded === o.id ? null : o.id,
                                        )
                                    }
                                >
                                    {expanded === o.id
                            ...
                            {expanded === o.id && (
                                <div className="pt-8 border-t border-border/50 space-y-4 animate-in fade-in slide-in-from-top-4 duration-500">
                                    ...
                                    {o.items.map((it) => (
                                        <div
                                            key={it.id}
                                            className="flex justify-between items-center group/item p-4 rounded-xl hover:bg-muted/50 transition-colors"
                                        >
